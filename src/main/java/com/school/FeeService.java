...
class FeeService {
  ...
  Long scope(AppUser u, Long requested) {
    if (u.role == Role.ADMIN) {
      if (requested == null)
        throw new IllegalArgumentException("schoolId is required for an administrator");
      return requested;
    }
    return u.school.id;
  }
  ...
  ReceiptResponse create(Authentication auth, Long requestedSchoolId, CreateFeeRequest request) {
    ...
    Student student =
        students
            .findByIdAndSchoolId(request.studentId(), schoolId)
            .filter(s -> s.active)
            .orElseThrow(() -> new NoSuchElementException("Active student not found"));
    if (request.items().stream().map(FeeItemRequest::feeTypeId).distinct().count()
        != request.items().size())
      throw new IllegalArgumentException("A fee type may appear only once");
    BigDecimal total =
        request.items().stream()
            .map(FeeItemRequest::amount)
    ...
    for (FeeItemRequest i : request.items())
      feeTypes.add(
          types
              .findByIdAndSchoolIdAndActiveTrue(i.feeTypeId(), schoolId)
              .orElseThrow(() -> new IllegalArgumentException("Invalid fee type")));
    Integer year = request.paymentDate().getYear();
    Number serial =
        (Number)
            em.createNativeQuery(
                    "INSERT INTO receipt_number_sequences(school_id,receipt_year,last_value) VALUES (?1,?2,1) ON CONFLICT(school_id,receipt_year) DO UPDATE SET last_value=receipt_number_sequences.last_value+1 RETURNING last_value")
    ...
    for (int i = 0; i < request.items().size(); i++) {
      ...
      item.feeType = feeTypes.get(i);
      item.amount = request.items().get(i).amount();
  ...
  ReceiptResponse cancel(Authentication auth, Long id, CancelRequest request, Long schoolId) {
    ...
    if (user.role != Role.ADMIN)
      throw new AccessDeniedException("Only administrators may cancel receipts");
    FeeReceipt receipt =
        receipts
            .findByIdAndSchoolId(id, scope(user, schoolId))
    ...
    receipt.cancellationReason = request.reason();
    receipt.cancelledAt = Instant.now();
    receipt.cancelledBy = user;
    audit(
        user,
        receipt.school,
        "CANCEL_FEE",
  ...
  ReceiptResponse view(FeeReceipt r) {
    return new ReceiptResponse(
        ...
        r.items.stream()
            .map(
                i ->
                    new ItemResponse(i.feeType.id, i.feeType.code, i.feeType.displayName, i.amount))
            .toList());
  }

  void audit(AppUser u, School s, String action, String type, Long id, String oldV, String newV) {
    ...
    audits.save(a);
=======
/** A positive payment amount for one configured fee type. */
record FeeItemRequest(@NotNull Long feeTypeId, @NotNull @DecimalMin("0.01") @Digits(integer=10,fraction=2) BigDecimal amount) {}
/** Input required to create one atomic, multi-item payment receipt. */
record CreateFeeRequest(@NotNull Long studentId, @NotNull LocalDate paymentDate, @NotEmpty List<@Valid FeeItemRequest> items) {}
/** Reason recorded when an administrator cancels a receipt. */
record CancelRequest(@NotBlank @Size(max=500) String reason) {}
/** Receipt-item representation returned to API clients. */
record ItemResponse(Long feeTypeId,String feeTypeCode,String feeTypeName,BigDecimal amount) {}
/** Complete read model for a receipt and its immutable fee items. */
...
/** Enforces fee ownership, receipt-number allocation, immutability, and auditing. */
