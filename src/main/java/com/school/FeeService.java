...
class FeeService {
  ...
  FeeService(
      ...
      AuditRepository a,
      EntityManager e,
      ClassFeeStructureRepository structures,
  ...
  ReceiptResponse create(Authentication auth, Long requestedSchoolId, CreateFeeRequest request) {
    ...
    for (FeeItemRequest i : request.items()) {
      configured.add(
          structures
              .findBySchoolIdAndSchoolClass_IdAndFeeType_IdAndAcademicYear(
                  schoolId, student.schoolClass.id, i.feeTypeId(), academicYear)
              .filter(x -> x.active)
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "Fee is not configured for this class and academic year")));
    }
    BigDecimal total =
        configured.stream().map(x -> x.amount).reduce(BigDecimal.ZERO, BigDecimal::add);
    if (total.signum() <= 0)
      throw new IllegalArgumentException("Selected fee total must be positive");
    ...
    for (ClassFeeStructure configuredFee : configured) {
      FeeReceiptItem item = new FeeReceiptItem();
      item.receipt = receipt;
      item.feeType = configuredFee.feeType;
      item.amount = configuredFee.amount;
  ...
  ReceiptResponse cancel(Authentication auth, Long id, CancelRequest request, Long schoolId) {
    ...
    audit(
        ...
        id,
        "{\"status\":\"ACTIVE\"}",
        "{\"status\":\"CANCELLED\",\"reason\":\"" + request.reason().replace("\"", "\\\"") + "\"}");
    return view(receipt);
  }

  ReceiptResponse view(FeeReceipt r) {
    String cn = r.student.schoolClass != null ? r.student.schoolClass.name : "";
    String sec = r.student.schoolClass != null ? r.student.schoolClass.section : "";
  ...
  static String academicYear(LocalDate d) {
