...
interface ClassRepository extends JpaRepository<SchoolClass, Long> {
  ...
  @Query("select c from SchoolClass c where c.school.id = :schoolId and c.active = true order by case lower(c.name) when 'nursery' then 0 when 'lkg' then 1 when 'ukg' then 2 when '1' then 3 when '2' then 4 when '3' then 5 when '4' then 6 when '5' then 7 when '6' then 8 when '7' then 9 when '8' then 10 when '9' then 11 when '10' then 12 when '11' then 13 when '12' then 14 else 1000 end, c.section")
...
interface ClassFeeStructureRepository extends JpaRepository<ClassFeeStructure, Long> {
  @Query("select c from ClassFeeStructure c "
