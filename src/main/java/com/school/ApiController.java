...
class ApiController {
  ...
  private final FeeStructureService feeStructures;
  ...
  ApiController(
      ...
      ReceiptRepository r,
      SchoolRepository sc,
      UserManagementService um,
      FeeStructureService fs) {
    fees = f;
    classes = c;
    students = s;
    types = t;
  ...
  UserResponse me(Authentication a) {
    return userManagement.view(fees.actor(a));
  }

  @GetMapping("/schools/{id}")
  School getSchool(Authentication a, @PathVariable Long id) {
    AppUser u = fees.actor(a);
    if (u.role != Role.ROOT && (u.school == null || !u.school.id.equals(id)))
  ...
  List<School> schools() {
    return schools.findByOrderByName();
  }

  @PostMapping("/schools")
  @PreAuthorize("hasRole('ROOT')")
  @ResponseStatus(HttpStatus.CREATED)
  School createSchool(Authentication a, @Valid @RequestBody CreateSchoolRequest q) {
    return userManagement.createSchool(a, q);
  }

  @GetMapping("/classes")
  @PreAuthorize("hasAnyRole('SCHOOL_ADMIN','SCHOOL_USER')")
  List<SchoolClass> listClasses(Authentication a, @RequestParam(required = false) Long schoolId) {
    return classes.findBySchoolIdOrderByNameAscSectionAsc(school(a, schoolId));
  }
  ...
  SchoolClass createClass(
    ...
    c.school = sc;
    c.name = q.name();
    c.section = q.section() == null ? "" : q.section();
  ...
  List<StudentResponse> listStudents(
    ...
    return students.findBySchoolId(school(a, schoolId)).stream()
        .map(
            s ->
                new StudentResponse(
                    s.id,
                    ...
                    s.phone,
                    s.active,
                    new ClassRef(s.schoolClass.id, s.schoolClass.name, s.schoolClass.section)))
        .toList();
  }
  ...
  StudentResponse createStudent(
    ...
    return new StudentResponse(
        ...
        saved.phone,
        saved.active,
        new ClassRef(c.id, c.name, c.section));
  }
  ...
  StudentResponse updateStudent(
    ...
    return new StudentResponse(
        ...
        saved.phone,
        saved.active,
        sc != null ? new ClassRef(sc.id, sc.name, sc.section) : null);
  }
  ...
  FeeType createType(
      Authentication a,
      @RequestParam(required = false) Long schoolId,
      @Valid @RequestBody FeeTypeRequest q) {
  ...
  @PreAuthorize("hasAnyRole('SCHOOL_ADMIN','SCHOOL_USER')")
  List<FeeStructureResponse> feeStructure(
      Authentication a,
      @RequestParam String academicYear,
      @RequestParam(required = false) Long schoolId) {
    return feeStructures.list(a, academicYear);
  }

  @PutMapping("/fee-structure")
  @PreAuthorize("hasRole('SCHOOL_ADMIN')")
  FeeStructureResponse updateFeeStructure(
      Authentication a, @Valid @RequestBody FeeStructureItemRequest q) {
    return feeStructures.upsert(a, q);
  }

  @GetMapping("/users")
  @PreAuthorize("hasRole('SCHOOL_ADMIN')")
  List<UserResponse> users(Authentication a) {
    return userManagement.listSchoolUsers(a);
  }

  @PostMapping("/users")
  @PreAuthorize("hasRole('SCHOOL_ADMIN')")
  @ResponseStatus(HttpStatus.CREATED)
  UserResponse createUser(Authentication a, @Valid @RequestBody CreateUserRequest q) {
    return userManagement.createSchoolUser(a, q);
  }

  @PatchMapping("/users/{id}")
  @PreAuthorize("hasRole('SCHOOL_ADMIN')")
  UserResponse setUserActive(
      Authentication a, @PathVariable Long id, @RequestParam boolean active) {
    return userManagement.setSchoolUserActive(a, id, active);
  }
