...
@Table(name = "schools")
class School extends Timestamped {
  @Id
...
@JsonAutoDetect(fieldVisibility = JsonAutoDetect.Visibility.ANY)
@Table(name = "users")
class AppUser extends Timestamped {
  @Id
  @GeneratedValue(strategy = GenerationType.IDENTITY)
  Long id;
...
@JsonAutoDetect(fieldVisibility = JsonAutoDetect.Visibility.ANY)
@Table(name = "classes")
class SchoolClass extends Timestamped {
  @Id
  @GeneratedValue(strategy = GenerationType.IDENTITY)
  Long id;
...
@JsonAutoDetect(fieldVisibility = JsonAutoDetect.Visibility.ANY)
@Table(name = "students")
class Student extends Timestamped {
  @Id
  @GeneratedValue(strategy = GenerationType.IDENTITY)
  Long id;
...
class FeeReceipt {
  ...
  List<FeeReceiptItem> items = new ArrayList<>();
}

@Entity
@Table(name = "fee_receipt_items")
...
class AuditLog {
  ...
  Instant createdAt = Instant.now();
=======
/** Authorization roles supported by the MVP. */
enum Role { ADMIN, SCHOOL }
/** Lifecycle state of a financial receipt; cancelled receipts remain immutable. */
enum ReceiptStatus { ACTIVE, CANCELLED }

/** Supplies creation and update timestamps to mutable domain records. */
...
/** A school that owns users, classes, students, fee types, and financial data. */
...
/** Authenticated staff account scoped to a school unless it is an administrator. */
...
/** A named class and section within one school. */
...
/** A student enrolled in a class and owned by exactly one school. */
...
/** Configurable category of charge, such as institute or van fee. */
...
/** Immutable payment header; corrections are recorded through cancellation and a replacement receipt. */
...
/** Individual fee-type amount belonging to one payment receipt. */
...
/** Append-only audit event describing a significant change to a domain record. */
